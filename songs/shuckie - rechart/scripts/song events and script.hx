...
var topBlackBar = new FunkinSprite(0, 0).makeSolid(FlxG.width, FlxG.height, FlxColor.BLACK);
var bottomBlackBar = new FunkinSprite(0, 0).makeSolid(FlxG.width, FlxG.height, FlxColor.BLACK);
topBlackBar.y = downscroll ? 570 : 530;
bottomBlackBar.y =  downscroll ? -530 : -570;

topBlackBar.alpha = bottomBlackBar.alpha = missWarn.alpha = 0;
...
function postCreate()
{
    ...
    strumLines.members[0].characters[0].cameraOffset.set(-2100, -170);
    ...
    camHUD.y = downscroll ? -100 : 100;

    camNote.alpha = 0;
    camHUD.alpha = 0;
    strumLines.members[0].characters[1].alpha = strumLines.members[1].characters[1].alpha = camHUD.alpha = camNote.alpha = 0;
